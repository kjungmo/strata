...
namespace prism_map {
prism_map_core::Observation scanToObservation(const sensor_msgs::msg::LaserScan&,
    const prism_map_core::Pose3D&, Eigen::Vector3d&);
}
TEST(ScanAdapter, SingleBeamUnderYawTranslation) {
  ...
  prism_map_core::Pose3D iso = prism_map_core::Pose3D::Identity();
  ...
  auto obs = prism_map::scanToObservation(scan, iso, origin);
