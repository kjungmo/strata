...
MappingNode::MappingNode(const rclcpp::NodeOptions& options)
    : rclcpp::Node("prism_map", options) {
  backend_ = declare_parameter<std::string>("backend", "grid2d");
  global_frame_ = declare_parameter<std::string>("global_frame", "map");
  save_path_ = declare_parameter<std::string>("save_path", "/tmp/prism_map");
  const double publish_period = declare_parameter<double>("publish_period", 1.0);

  const prism_map_core::LayeredMapParams lp = readLayerParams();

  tf_buffer_ = std::make_unique<tf2_ros::Buffer>(get_clock());
  tf_listener_ = std::make_shared<tf2_ros::TransformListener>(*tf_buffer_);

  if (backend_ == "grid2d") {
    prism_map_core::GridMeta meta;
    ...
    grid_ = std::make_unique<prism_map_core::Grid2DBackend>(meta, lp);
    ...
    voxel_ = std::make_unique<prism_map_core::Voxel3DBackend>(voxel_size, lp);
  ...
  RCLCPP_INFO(get_logger(), "prism_map up: backend=%s frame=%s", backend_.c_str(),
              global_frame_.c_str());
}

prism_map_core::LayeredMapParams MappingNode::readLayerParams() {
  prism_map_core::LayeredMapParams p;
...
void MappingNode::onScan(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
  ...
  const prism_map_core::Pose3D iso = tf2::transformToEigen(tf);
...
void MappingNode::onPublish() {
  ...
  if (backend_ == "grid2d") {
    if (!grid_) return;
    const prism_map_core::GridMap g = grid_->toOccupancyGrid();
